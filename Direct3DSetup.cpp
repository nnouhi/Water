...
ID3D11Texture2D*        gBackBufferTexture      = nullptr;
...
bool InitDirect3D()
{
    ...
    hr = gSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&gBackBufferTexture);
    if (FAILED(hr))
    {
        gLastError = "Error creating swap chain";
        return false;
    }
    hr = gD3DDevice->CreateRenderTargetView(gBackBufferTexture, NULL, &gBackBufferRenderTarget);
    if (FAILED(hr))
    {
...
void ShutdownDirect3D()
{
	...
	if (gBackBufferTexture)      gBackBufferTexture->Release();
